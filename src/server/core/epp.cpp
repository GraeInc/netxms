...
EPRule::EPRule(const ConfigEntry& config) : m_timeFrames(0, 16, Ownership::True), m_actions(0, 16, Ownership::True)
{
   ...
   {
      unique_ptr<ObjectArray<ConfigEntry>> frames = timeFrameRoot->getSubEntries(_T("frame#*"));
...
void EPRule::createExportRecord(StringBuffer &xml) const
{
   ...
   {
      auto pair = m_timeFrames.get(i);
      xml.appendFormattedString(_T("\t\t\t\t<frame id=\"%d\" time=\"%d\" date=\"%ld\" />\n"), i + 1, pair->getTime(), pair->getDate());
   }
...
bool EPRule::loadFromDB(DB_HANDLE hdb)
{
   ...
   // Load rule's events
   _sntprintf(szQuery, 256, _T("SELECT time_filter,date_filter FROM policy_time_list WHERE rule_id=%d"), m_id);
   hResult = DBSelect(hdb, szQuery);
   if (hResult != nullptr)
   {
      int count = DBGetNumRows(hResult);
...
bool EPRule::saveToDB(DB_HANDLE hdb) const
{
   ...
   {
      hStmt = DBPrepare(hdb, _T("INSERT INTO policy_time_list (rule_id,time_frame_id,time_filter,date_filter) VALUES (?,?,?,?)"), m_pstorageSetActions.size() > 1);
      if (hStmt != nullptr)
      {
         DBBind(hStmt, 1, DB_SQLTYPE_INTEGER, m_id);
         int i = 0;
...
void EPRule::createMessage(NXCPMessage *msg) const
{
   ...
   {
      auto pair = m_timeFrames.get(i);
      msg->setField(fieldId++, pair->getTime());
      msg->setField(fieldId++, pair->getDate());
   }
...
json_t *EPRule::toJson() const
{
   ...
   json_object_set_new(root, "frames", timeFrames);
...
bool EventPolicy::saveToDB() const
{
   ...
   if (success)
   {
      success = DBQuery(hdb, _T("DELETE FROM event_policy")) &&
                DBQuery(hdb, _T("DELETE FROM policy_time_list")) &&
