...
public class TimeFrame
{
   ...
   {
      if (isNever())
         return getText("Never", locale);
   
      StringBuilder builder = new StringBuilder();   
      
      if (isAnyTime())
      {
         builder.append(getText("Any time", locale));         
      }
      else
      {
         ...
         builder.append(String.format(getText("From %s to %s", locale), dfTime.format(from.getTime()), dfTime.format(to.getTime())));
      }
      
      if (isAnyDay())
      {
         builder.append(getText(" any day", locale));   
      }
      else
      {
         builder.append(String.format(getText(" on %s on %s in %s", locale), getDaysOfMonth(), 
         ...
         switch(state)
         {
            case 0:
               if (bits[i])
               {
                  if (text.length() != 0)
                     text.append(", ");
                  text.append(getText(options[i], locale));
                  state = 1;
               }
               break;
            case 1:
   ...
   {    
            ...
            startMinute < 0 || startMinute > 59  || endMinute < 0 || endMinute > 59)
      {
         throw new TimeRangeFormatException(TimeRangeFormatException.TIME_VALIDATION_FAILURE, 
               getText("Incorrect time. Hours should be between 0 and 23 and minutes between 1 and 59."));
      }
      if (startTime > endTime)
      {
         throw new TimeRangeFormatException(TimeRangeFormatException.TIME_INCORRECT_ORDER, 
               getText("Start time should be less than end time."));
      } 
               ...
               try
               {
                  ...
                  else
                  {
                     throw new TimeRangeFormatException(TimeRangeFormatException.DAY_OUT_OF_RANGE, 
                           getText("Only 1 to 31 dates are allowed."));
                  }
               }
               catch (Exception e)
               {
                  throw new TimeRangeFormatException(TimeRangeFormatException.DAY_NOT_A_NUMBER, 
                        getText("Only 1 to 31 dates and \"L\" symbol are allowed."));
               }
            }
    ...
    * @return message text
    */
   public static String getText(String text, Locale locale)
   {
      try
      {
